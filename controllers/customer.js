...
module.exports.hotelData = async (req, res) => {
  try {
    ...
    if (!customerData || !hotelData) {
      return res.status(404).send({ error: "Customer or Hotel data not found." });
    }

    // Extracting default location
    const defaultLocation = customerData.location?.find(loc => loc.isDefault === true);
    if (!defaultLocation) {
      return res.status(404).send({ error: "Default location not found for the customer." });
    }
...
module.exports.liveOrder = async (req, res) => {
  try {
    ...
    if (!data || data.length === 0) {
      return res.status(404).json({ message: "No past orders found for this customer." });
    }
    ...
    const dataWithDistanceAndTime = data.map((order) => {
...
module.exports.paymentConfirm = async (req, res) => {
  try {
    ...
    const payment_Data = await PaymentLog.findByIdAndUpdate(
      paymentId,
      { status: "SUCCESS" },
      { new: true }
    );
    ...
    if (!payment_Data) {
      return res.status(404).json({
        status: "Failure",
        message: `Payment record not found for ID: ${paymentId}`,
      });
    }

    // Map order items to the schema structure
    const orderData = {
      ticketNumber,
      orderOtp,
      customer: user_id,
    ...
    // Save the entire order in one go
    await LiveOrder.create(orderData);
